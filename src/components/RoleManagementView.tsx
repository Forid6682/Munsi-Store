...
export const RoleManagementView: React.FC<RoleManagementViewProps> = ({
  ...
  const isAdmin = currentUser?.role === 'admin' || currentUser?.email === 'ahmedmdforid39@gmail.com';
  ...
  const loadUsers = async () => {
    try {
      ...
      if (list.length > 0) {
        ...
        // Fallback local initial list including the admin and demo SR/DSR
        setUsers([
          {
            uid: currentUser?.uid || 'usr-admin-1',
            email: currentUser?.email || 'ahmedmdforid39@gmail.com',
            displayName: currentUser?.displayName || 'ফরহাদ আহমদ (ডিস্ট্রিবিউটর)',
            role: 'admin',
          ...
          {
            uid: 'usr-sr-1',
            email: 'sr.karim@munsistore.com',
            displayName: 'মো: রেজাউল করিম (SR)',
            role: 'sr',
            assignedRoute: 'মিরপুর রুট',
            phone: '01719887766',
            status: 'active',
          },
          {
            uid: 'usr-dsr-1',
            email: 'dsr.habib@munsistore.com',
            displayName: 'হাবিবুর রহমান (DSR)',
            role: 'dsr',
            assignedRoute: 'চকবাজার রুট',
            phone: '01822554433',
            status: 'active',
          },
        ]);
