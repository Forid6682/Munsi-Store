...
export const OrderBookingView: React.FC<OrderBookingViewProps> = ({
  ...
  const handleVoiceOrderParsed = (parsed: {
    shopId?: string;
    ...
    if (parsed.shopId) {
      setSelectedShopId(parsed.shopId);
    }
    if (parsed.items && parsed.items.length > 0) {
      const newCart = { ...cart };
      parsed.items.forEach((it) => {
        newCart[it.productId] = (newCart[it.productId] || 0) + it.quantity;
      });
      setCart(newCart);
    }
            ...
            <div className="flex items-center justify-between mb-1.5">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="flex items-center gap-1 px-2.5 py-1 bg-purple-50 hover:bg-purple-100 text-purple-800 border border-purple-200 rounded-lg text-xs font-semibold shadow-xs"
                >
                  <Sparkles className="w-3.5 h-3.5 text-purple-600" />
                  <span>ভয়েস / এআই অর্ডার</span>
                </button>
                <button
